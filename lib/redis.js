...
export async function recordDevice(userId, fingerprint, username, displayName) {
  const data = await getDeviceData();
  const entry = data[String(userId)] || { fingerprints: [], flagged: false, blocked: false, username, displayName };
  if (!entry.fingerprints.includes(fingerprint)) {
    entry.fingerprints.push(fingerprint);
    entry.username = username;
    entry.displayName = displayName;
    // Flag if more than 3 unique fingerprints (different devices)
    if (entry.fingerprints.length > 3) entry.flagged = true;
  }
...
export async function setUserBlocked(userId, blocked) {
  const data = await getDeviceData();
  if (!data[String(userId)]) data[String(userId)] = { fingerprints: [], flagged: false, blocked, username: '', displayName: '' };
  data[String(userId)].blocked = blocked;
  data[String(userId)].flagged = blocked ? data[String(userId)].flagged : false;
  await saveDeviceData(data);
}
