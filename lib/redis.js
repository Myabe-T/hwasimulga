...
export const redis = {
  ...
  set:      (key, value, opts)       => opts?.ex
                                          ? upstash(['SET', key, value, 'EX', opts.ex])
                                          : upstash(['SET', key, value]),
  del:      (key)                    => upstash(['DEL', key]),
  lpush:    (key, ...values)         => upstash(['LPUSH', key, ...values]),
  rpush:    (key, ...values)         => upstash(['RPUSH', key, ...values]),
  ltrim:    (key, start, stop)       => upstash(['LTRIM', key, start, stop]),
  lrange:   (key, start, stop)       => upstash(['LRANGE', key, start, stop]),
  sadd:     (key, ...members)        => upstash(['SADD', key, ...members]),
  srem:     (key, ...members)        => upstash(['SREM', key, ...members]),
  smembers: (key)                    => upstash(['SMEMBERS', key]),
  // HASH operations — consolidate scattered keys into single HASH keys
  hget:     (key, field)             => upstash(['HGET', key, field]),
  hdel:     (key, ...fields)         => upstash(['HDEL', key, ...fields]),
  hkeys:    (key)                    => upstash(['HKEYS', key]),
  hgetall:  (key)                    => upstash(['HGETALL', key]),
  hincrby:  (key, field, inc)        => upstash(['HINCRBY', key, field, String(inc)]),
  // hset accepts an object: { field: value, ... }
  hset: (key, obj) => {
    const args = ['HSET', key];
...
export const KEYS = {
  ...
  THUMBS:        'hwasi:thumbs',        // HASH { id → base64 }
  BOOKMARKS:     'hwasi:bookmarks',     // HASH { userId → JSON([ids]) }
  PREMIUM:       'hwasi:premium',       // HASH { userId → JSON({...}) }
  NOTIFICATIONS: 'hwasi:notifications', // HASH { userId → JSON({...}) }
  SHARES:        'hwasi:shares',        // HASH { token → JSON({...}) }
  // Legacy aliases (kept for zero-downtime migration)
...
export const DEFAULT_SETTINGS = { start: 51, end: 730, cdnId: 'desimms', extraRanges: [] };
...
// ── Thumbnail storage (HASH: hwasi:thumbs → { id: base64 }) ─────────────────
export async function getThumbnailB64(id) {
  // Try new HASH format first, fall back to old STRING key
  let val = await redis.hget(KEYS.THUMBS, String(id));
  if (!val) val = await redis.get(`hwasi:thumb:${id}`).catch(() => null);
  if (!val) return null;
  if (typeof val === 'string' && val.startsWith('data:')) return val.split(',')[1] || null;
  return typeof val === 'string' ? val : null;
}
export async function getThumbnail(id) { return getThumbnailB64(id); }
export async function setThumbnail(id, dataUrl) {
  const b64 = typeof dataUrl === 'string' && dataUrl.includes(',') ? dataUrl.split(',')[1] : dataUrl;
  await redis.hset(KEYS.THUMBS, { [String(id)]: b64 });
}
export async function deleteThumbnail(id) {
  await redis.hdel(KEYS.THUMBS, String(id));
}
export async function getThumbnailIds() {
  return await redis.hkeys(KEYS.THUMBS);
}

// ── Premium subscriptions (HASH: hwasi:premium → { userId: JSON }) ─────────
export async function getPremium(userId) {
  const raw = await redis.hget(KEYS.PREMIUM, String(userId));
  if (!raw) return null;
  const data = typeof raw === 'string' ? JSON.parse(raw) : raw;
  if (data.expiresAt && new Date(data.expiresAt) < new Date()) {
    await redis.hdel(KEYS.PREMIUM, String(userId));
    return null;
  }
  return data;
}
export async function setPremium(userId, plan, days, addedBy = 'admin') {
  ...
  await redis.hset(KEYS.PREMIUM, { [String(userId)]: JSON.stringify(rec) });
}
export async function revokePremium(userId) {
  await redis.hdel(KEYS.PREMIUM, String(userId));
}
export async function getAllPremiumIds() {
  return await redis.hkeys(KEYS.PREMIUM);
}

// ── Daily view counts (HASH per day: hwasi:views:{date} → { trackingKey: count }) ─
function todayHashKey() {
  return `hwasi:views:${new Date().toISOString().slice(0, 10)}`;
}
export async function getViewCount(trackingKey) {
  const v = await redis.hget(todayHashKey(), trackingKey);
  return v ? Number(v) : 0;
}
export async function incrementViewCount(trackingKey) {
  const hashKey = todayHashKey();
  const env = getEnv();
  const url = env.UPSTASH_REDIS_REST_URL;
  const token = env.UPSTASH_REDIS_REST_TOKEN;
  // HINCRBY + EXPIRE on the whole day-hash (25h TTL)
  const res = await fetch(`${url}/pipeline`, {
    method: 'POST',
    headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
    body: JSON.stringify([['HINCRBY', hashKey, trackingKey, '1'], ['EXPIRE', hashKey, 90000]]),
  });
...
export const PLANS = {
  basic: { label: 'Basic',  price: 100,  days: 20,         color: '#7c3aed', originalPrice: 200 },
  plus:  { label: 'Plus',   price: 300,  days: 90,         color: '#0ea5e9', originalPrice: 500 },
  pro:   { label: 'Pro',    price: 599,  days: 730,        color: '#f59e0b', originalPrice: 999 },
};

// ── Bookmarks (HASH: hwasi:bookmarks → { userId: JSON([ids]) }) ─────────────
export async function getBookmarks(userId) {
  const raw = await redis.hget(KEYS.BOOKMARKS, String(userId));
  if (!raw) return [];
  try { return (typeof raw === 'string' ? JSON.parse(raw) : raw).map(Number); } catch { return []; }
}
export async function addBookmark(userId, videoId) {
  const ids = await getBookmarks(userId);
  const vid = Number(videoId);
  if (!ids.includes(vid)) ids.push(vid);
  await redis.hset(KEYS.BOOKMARKS, { [String(userId)]: JSON.stringify(ids) });
}
export async function removeBookmark(userId, videoId) {
  const ids = await getBookmarks(userId);
  const filtered = ids.filter(id => id !== Number(videoId));
  await redis.hset(KEYS.BOOKMARKS, { [String(userId)]: JSON.stringify(filtered) });
}
...
export async function clearDevice(userId, fingerprint) {
  const data = await getDeviceData();
  const entry = data[String(userId)];
  if (!entry || !entry.devices) return;
  delete entry.devices[fingerprint];
  // Recalculate flagged status
  const deviceCount = Object.keys(entry.devices).length;
  if (deviceCount <= 3) entry.flagged = false;
  data[String(userId)] = entry;
  await saveDeviceData(data);
}
export async function clearAllDevices(userId) {
  const data = await getDeviceData();
  if (data[String(userId)]) {
    data[String(userId)].devices = {};
    data[String(userId)].flagged = false;
  }
  await saveDeviceData(data);
}
